...
import static no.minecraft.world.Chunk.*;
import static org.lwjgl.opengl.GL11.*;
import static org.lwjgl.opengl.GL15.*;
import static org.lwjgl.opengl.GL20.*;
import static org.lwjgl.opengl.GL30.*;

/** Mesh geometry and GPU resources, used only on the thread with the active GL context. */
final class ChunkMesh {
    ...
    public void rebuildMesh() {
        ...
        for (int dx = -1; dx <= 1; dx++) {
            for (int dz = -1; dz <= 1; dz++) {
                Chunk neighbor = (dx == 0 && dz == 0) ? chunk : world.getChunk(chunk.getChunkX() + dx, chunk.getChunkZ() + dz);
                if (neighbor != null) {
        ...
        for (int y = 0; y < SIZE_Y; y++) {
            for (int z = 0; z < SIZE_Z; z++) {
                for (int x = 0; x < SIZE_X; x++) {
                    ...
                    float wx = chunk.getWorldStartX() + x;
                    float wy = y;
                    float wz = chunk.getWorldStartZ() + z;
    ...
    private float skyExposure(int x, int y, int z, BlockType.Face face) {
        ...
        int lx = nx - chunk.getWorldStartX();
        int lz = nz - chunk.getWorldStartZ();
    ...
    private void addCactus(List<Float> v, float wx, float wy, float wz, int x, int y, int z, float torchLight) {
        ...
        // Top face (+Y)
        if (chunk.getBlock(x, y + 1, z) != BlockType.CACTUS) {
